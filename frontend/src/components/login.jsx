...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          navigate("/");
          window.location.reload(); // Forces a full page reload to get updated data
        }, 2000);
  ...
  const handleGoogleLogin = () => {
    window.location.href = "http://localhost:3000/auth/google"; // Redirect to backend Google OAuth
  };
...
//     } else if (field === "photoReq") {
//       updatedPuzzles[index].photoReq = value;
//     } else {
...
//     setFormData({
//       ...formData,
//       puzzles: [...formData.puzzles, { puzzleText: "", location: { coordinates: ["", ""] }, hints: [{ hint: "" }], photoReq: false }],
//     });
//     setActiveTab(formData.puzzles.length);
//   };

//   const handleRemovePuzzle = (index) => {
//     const updatedPuzzles = [...formData.puzzles];
//     updatedPuzzles.splice(index, 1);
//     setFormData({ ...formData, puzzles: updatedPuzzles });
//     if (activeTab >= updatedPuzzles.length) {
//       setActiveTab(Math.max(0, updatedPuzzles.length - 1));
//     }
//   };
...
//   const handleRemoveHint = (puzzleIndex, hintIndex) => {
//     const updatedPuzzles = [...formData.puzzles];
//     updatedPuzzles[puzzleIndex].hints.splice(hintIndex, 1);
//     setFormData({ ...formData, puzzles: updatedPuzzles });
//   };
...
//   const validateForm = () => {
//     if (!formData.name.trim() || !formData.description.trim()) return "Name and description are required.";
//     if (!formData.startTime || !formData.endTime) return "Start and end time are required.";
//     if (new Date(formData.startTime) >= new Date(formData.endTime)) return "Start time must be before end time.";
//     if (formData.puzzles.length === 0) return "At least one puzzle is required.";
//     for (const puzzle of formData.puzzles) {
//       if (!puzzle.puzzleText.trim()) return "Each puzzle must have a question.";
//       if (!puzzle.location.coordinates[0] || !puzzle.location.coordinates[1]) return "Each puzzle must have valid coordinates.";
//       if (puzzle.hints.length === 0 || puzzle.hints.some((hint) => !hint.hint.trim())) return "Each puzzle must have at least one hint.";
//     }
//     return "";
//   };
...
//       setSuccess("Hunt created successfully!");
//       setFormData({
//         name: "",
//         description: "",
//         startTime: "",
